 ...
 *
 * Assume Case: You want a task that adapts to the actual time it spends processing, while still reacting to external events. 
 * The task must maintain consistent intervals between cycles, adjusting for the time spent handling events or other work.
 * 
 * How to Solve: Track the time taken by each task cycle, and adjust the waiting time accordingly. 
 * By subtracting the task execution time from the total period, 
 * the task maintains a relatively uniform cycle time, even when reacting to external stimuli.
*/
